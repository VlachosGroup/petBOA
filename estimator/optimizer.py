...
class ModelWrapper():
    ...
    def loss_func(self, xi):
        """Generic loss function"""
        loss = 0
        X,Y = np.meshgrid(self.x_inputs[0],self.x_inputs[1])
        Z = self.model((X,Y),xi)
        y_predict = Z.reshape(1, -1)
        # print(y_predict.shape,self.y_groundtruth.shape)
        # for i in range(self.n_trials):
        #     # Factor in the weights
        #     loss += WeightedRMSE(self.y_groundtruth[0][i], y_predict[0][i], self.y_weights)
        for i in range(self.n_responses):
            # Factor in the weights
            loss += WeightedRMSE(self.y_groundtruth[i], y_predict[0], self.y_weights)
        return loss
